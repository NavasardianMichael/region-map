import type { CommonNs } from './types';

const ru: CommonNs = {
  appName: 'Карта регионов',
  nav: {
    home: 'Главная',
    projects: 'Проекты',
    contact: 'Контакты',
    faq: 'Частые вопросы',
    pricing: 'Тарифы',
    about: 'О приложении',
    login: 'Войти',
    account: 'Аккаунт',
    logout: 'Выйти',
    deleteAccount: 'Удалить аккаунт',
    cancel: 'Отмена',
    logoutError: 'Не удалось выйти. Попробуйте ещё раз.',
    languageSelectAriaLabel: 'Язык интерфейса',
    languageSwitchingOverlay: 'Смена языка',
    openMenu: 'Открыть меню',
    mainMenu: 'Меню',
    closeMenu: 'Закрыть меню',
  },
  deleteAccountModal: {
    title: 'Удалить аккаунт',
    content: 'Все данные, включая проекты, будут удалены безвозвратно.',
    ok: 'Удалить аккаунт',
    error: 'Не удалось удалить аккаунт',
  },
  home: {
    title: 'Regionify',
    welcome: 'Создавайте красивые интерактивные визуализации региональных карт.',
    heroHeadline: 'Превратите данные в красивые региональные карты',
    heroSubheadline:
      'Импортируйте данные из CSV, Excel, JSON или Google Sheets, настраивайте цвета, легенды и стили, а затем экспортируйте графику карт в готовом для публикации виде. Разовая оплата — никаких подписок.',
    ctaStart: 'Начать создание',
    ctaBadges: 'Смотреть тарифы',
    featuresTitle: 'Всё для визуализации региональных данных',
    featureImportTitle: 'Гибкий импорт данных',
    featureImportDesc:
      'Загружайте данные из CSV, Excel, JSON или подключайтесь напрямую к Google Sheets.',
    featureCustomizeTitle: 'Расширенные стили',
    featureCustomizeDesc:
      'Цветовые шкалы, настраиваемые легенды, форматирование подписей и полный контроль над внешним видом карты.',
    featureExportTitle: 'Высококачественный экспорт',
    featureExportDesc: 'Скачивайте карты в PNG, SVG или JPEG с качеством до 100 %.',
    featureTimeSeriesTitle: 'Временные ряды и анимация',
    featureTimeSeriesDesc:
      'Импортируйте исторические данные, перемещайтесь во времени и экспортируйте анимированные GIF или MP4.',
    mapsTitle: 'Более 200 карт включено',
    mapsSubtitle:
      'Страны, континенты, штаты США, регионы ЕС и многое другое — готовы к использованию.',
    dataModesTitle: 'Статические снимки и временные ряды',
    dataModesSubtitle:
      'Нужна разовая карта или полная историческая хронология — Regionify справится с обоими задачами.',
    dataModesStaticTitle: 'Статические данные',
    dataModesStaticDesc:
      'Импортируйте набор данных и создайте готовую к публикации хороплетную карту за секунды — идеально для отчётов, статей и презентаций.',
    dataModesTimeSeriesTitle: 'Исторические временные ряды',
    dataModesTimeSeriesDesc:
      'Загрузите данные за несколько периодов, перемещайтесь по интерактивной хронологии и наблюдайте за изменениями регионов — затем экспортируйте анимацию в GIF или MP4.',
    ctaBottomTitle: 'Готовы картировать свои данные?',
    ctaBottomSubtitle: 'Создайте первую визуализацию за минуты — кредитная карта не нужна.',
    showcaseTitle: 'Экспортируйте и делитесь картами',
    showcaseSvgTitle: 'Чёткий экспорт в SVG',
    showcaseSvgDesc:
      'Экспортируйте идеальные векторные карты, масштабируемые до любого размера — для печати, презентаций и профессиональных отчётов.',
    showcaseGifTitle: 'Анимированная GIF-хронология',
    showcaseGifDesc:
      'Превращайте временные ряды в зацикленные GIF-анимации, рассказывающие историю с первого взгляда.',
    showcaseMp4Title: 'Экспорт видео MP4',
    showcaseMp4Desc:
      'Экспортируйте плавные видеохронологии для презентаций, социальных сетей или видеопроектов.',
    showcasePublicPageTitle: 'Публичная страница с SEO',
    showcasePublicPageDesc:
      'Поделитесь своей интерактивной картой на отдельной публичной странице с настраиваемым заголовком, описанием и превью для поисковиков и ссылок.',
    showcaseEmbedTitle: 'Встраиваемый iframe',
    showcaseEmbedDesc:
      'Разместите свою интерактивную карту в любом месте сайта, блога или приложения с помощью простого iframe — полностью адаптивно и всегда актуально.',
  },
  about: {
    title: 'О Regionify',
    description:
      'Regionify — браузерный инструмент для хороплетных карт: подключайте CSV, Excel, JSON или Google Sheets к региональным SVG-картам, настраивайте легенды и цвета, экспортируйте изображения или анимации для отчётов и веба. Старшие тарифы добавляют временные ряды, экспорт GIF/MP4 и публичные встраиваемые страницы.',
  },
  faq: {
    title: 'Частые вопросы',
    intro:
      'Краткие ответы о хороплетных картах в Regionify: импорт, экспорт, тарифы, встраивание и сопоставление регионов.',
    items: [
      {
        question: 'Что такое Regionify?',
        answer:
          'Regionify — веб-приложение для интерактивных **хороплетных карт** из ваших данных. Вы выбираете региональную карту, импортируете значения из **CSV**, **Excel**, **JSON** или **Google Sheets**, настраиваете легенду и экспортируете изображения или анимации в зависимости от тарифа.',
      },
      {
        question: 'Что такое хороплетная карта?',
        answer:
          '**Хороплетная карта** раскрашивает или затеняет заданные **регионы** по числовому показателю — например плотность населения или продажи по штатам. **Regionify** закрывает импорт, сопоставление, стиль и экспорт.',
      },
      {
        question: 'Какие форматы можно импортировать?',
        answer:
          'Таблицы **CSV** и **Excel**, **JSON** и **Google Sheets** с живой синхронизацией на поддерживаемых тарифах. Regionify сопоставляет подписи с названиями регионов по **текстовому сходству**.',
      },
      {
        question: 'Какие форматы экспорта доступны?',
        answer:
          'Зависит от тарифа. Бесплатный **Observer** уже включает экспорт **JPEG**, **PNG** и **PDF**, расширенные стили карты и легенды, а также экспорт в высоком разрешении (2K/4K). Тариф **Исследователь** добавляет экспорт **SVG**, временные ряды со шкалой времени, анимационный экспорт (**GIF** и **MP4**) и ИИ-парсер данных. Тариф **Хронограф** включает всё, что есть в Исследователе, плюс публичное встраивание и более высокий дневной лимит ИИ-запросов.',
      },
      {
        question: 'Можно встроить карту на сайт?',
        answer:
          'Публичное встраивание и **iframe** доступны на тарифе **Хронограф**. Можно включить **публичную страницу** на проект с **SEO**-заголовком и описанием.',
      },
      {
        question: 'Это подписка?',
        answer:
          'Нет. Оба платных тарифа (**Исследователь** и **Хронограф**) — разовые платежи. Заплатите один раз и пользуйтесь бессрочно — никаких регулярных списаний, никакого срока действия.',
      },
      {
        question: 'Как работает сопоставление регионов?',
        answer:
          'После импорта Regionify сравнивает ваши подписи с именами регионов по **текстовому сходству**. Связи можно исправить в **визуализаторе**.',
      },
      {
        question: 'Нужен ли аккаунт?',
        answer:
          'Да. Аккаунты нужны для проектов, оплаты и опционального входа через Google. Актуальные лимиты — на странице **«Тарифы»**.',
      },
      {
        question: 'Как связаться с поддержкой?',
        answer:
          'Используйте страницу **«Контакты»** в приложении. По оплате см. страницу **«Тарифы»** с инструкцией, как написать нам.',
      },
    ],
  },
  badges: {
    title: 'Выберите бейдж',
    subtitle: 'Выберите уровень доступа — платите один раз, никаких регулярных платежей.',
    oneTimeTagline: 'Разовый платёж • Без подписки • Пожизненный доступ',
    secureCheckoutPrefix: '🔒 Безопасная оплата через',
    secureCheckoutDetails:
      '• Мы никогда не храним данные вашей карты • Принимаем основные кредитные карты, PayPal, Apple Pay и Google Pay.',
    paymentIssueNote: 'Если у вас возникли проблемы с оплатой, пожалуйста,',
    paymentContactUs: 'свяжитесь с нами',
    checkoutError: 'Не удалось начать оплату. Попробуйте ещё раз.',
    currentBadge: 'Текущий бейдж',
    bestChoice: 'Лучший выбор',
    priceFree: 'Бесплатно',
    priceOneTime: '{{price}} единоразово',
    priceOneTimeVatIncluded: '{{price}} единоразово (НДС включён)',
    localCurrencyNote: 'Цены отображаются в вашей местной валюте.',
    paymentPendingNote:
      'Платёж получен. Обновление бейджа может занять некоторое время. Обновите страницу или откройте раздел «Тарифы».',
    verifyErrorNote:
      'Не удалось подтвердить бейдж. Обновите страницу или откройте раздел «Тарифы».',
    backToBadges: 'Назад к тарифам',
    goToBadges: 'К тарифам',
    badgeUpgraded: 'Теперь вы используете значок {{badge}}.',
    paymentVerifying: 'Платёж обрабатывается. Обновляем ваш бейдж…',
    paymentCancelledTitle: 'Оплата отменена',
    paymentCancelledDesc: 'Вы отменили оплату. Средства не были списаны.',
    paymentSuccessTitle: 'Оплата прошла успешно',
    continueToProjects: 'Перейти к моим проектам',
    rows: {
      projectsLimited: 'До {{count}} сохранённых проектов',
      projectsUnlimited: 'Неограниченно сохранённых проектов',
      sessionsLimited: 'До {{count}} одновременных устройств',
      imageExport: 'Экспорт {{formats}}, качество до {{quality}}%',
      advancedStyles: 'Расширенные стили карты и легенды',
      watermark: 'Водяной знак на экспорте изображений',
      noWatermark: 'Без водяного знака на экспорте изображений',
      highResolutionExport: 'Экспорт в высоком разрешении (2K, 4K)',
      timeSeries: 'Временные ряды и шкала времени на карте',
      animationExport: 'Анимационный экспорт (GIF и MP4)',
      embedMapIframe: 'Встраивание карты (iframe)',
      publicMapPage: 'Публичная страница с картой',
      aiParser: 'ИИ-парсер и генератор данных (до {{count}} запросов в день)',
    },
    items: {
      observer: {
        name: 'Наблюдатель (бесплатно)',
        description:
          'Экспорт PNG и JPEG в полном качестве с водяным знаком, расширенные стили карты и легенды, экспорт в высоком разрешении до 4K и до 5 проектов.',
        buttonText: 'Бесплатный бейдж',
      },
      explorer: {
        name: 'Исследователь',
        description:
          'PNG, SVG и JPEG в полном качестве, без водяного знака, неограниченное количество проектов, расширенные стили, временные ряды с шкалой времени, анимационный экспорт в GIF/MP4 и ИИ-парсер данных (до 5 запросов в день).',
        buttonText: 'Купить Исследователь (разовый платёж)',
      },
      chronographer: {
        name: 'Хронограф',
        description:
          'Всё, что есть в Исследователе, плюс публичная страница и вставка через iframe для ваших проектов, а также более высокий лимит ИИ-запросов (до 10 в день).',
        buttonText: 'Купить Хронограф (разовый платёж)',
      },
    },
  },
  auth: {
    login: {
      title: 'С возвращением',
      subtitle: 'Войдите в аккаунт, чтобы продолжить',
      continueGoogle: 'Продолжить с Google',
      dividerEmail: 'или войти по эл. почте',
      password: 'Пароль',
      emailPlaceholder: 'email@example.com',
      passwordPlaceholder: 'Введите пароль',
      forgotPassword: 'Забыли пароль?',
      signIn: 'Войти',
      noAccount: 'Нет аккаунта?',
      signUpLink: 'Зарегистрироваться',
      resendVerification: 'Отправить письмо подтверждения снова',
      signInEvictDevices: 'Войти и выйти на всех других устройствах',
    },
    signUp: {
      title: 'Создать аккаунт',
      subtitle: 'Зарегистрируйтесь, чтобы начать работу с Regionify',
      verifyTitle: 'Подтвердите эл. почту',
      verifyBody:
        'Мы отправили ссылку для подтверждения на ваш адрес. Проверьте почту и перейдите по ссылке, чтобы завершить регистрацию.',
      verifyNote: 'После подтверждения вы сможете войти, используя свои учётные данные.',
      goToLogin: 'Перейти ко входу',
      continueGoogle: 'Продолжить с Google',
      dividerEmail: 'или зарегистрироваться по эл. почте',
      fullName: 'Полное имя',
      fullNamePlaceholder: 'Иван Иванов',
      password: 'Пароль',
      createPasswordPlaceholder: 'Придумайте пароль',
      confirmPassword: 'Подтвердите пароль',
      confirmPasswordPlaceholder: 'Повторите пароль',
      createAccount: 'Создать аккаунт',
      alreadyHaveAccount: 'Уже есть аккаунт?',
      signInLink: 'Войти',
      forgotPassword: 'Забыли пароль?',
    },
  },
  visualizer: {
    mapAreaTitle: 'Визуализация карты',
    renameProjectTitle: 'Переименовать проект',
    renameProjectPrompt: 'Введите новое имя:',
    renameProjectOk: 'Переименовать',
    mapHeaderRenameTooltip: 'Переименовать проект',
    mapHeaderDeleteTooltip: 'Удалить проект',
    mapHeaderLockTooltip: 'Заблокировать проект',
    mapHeaderUnlockTooltip: 'Разблокировать проект',
    mapHeaderLockedSaveTooltip:
      'Проект заблокирован. Разблокируйте его, чтобы сохранить изменения.',
    mapAriaMapOf: 'Карта: {{region}}',
    mapAriaNoCountrySelected: 'Страна не выбрана',
    tabData: 'Данные',
    tabMap: 'Карта',
    tabStyles: 'Стили',
    saveModalTitle: 'Сохранить проект',
    saveModalPrompt: 'Введите название проекта:',
    saveModalPlaceholder: 'Мой картографический проект',
    saveModalCreate: 'Создать',
    saveLoginToSave: 'Войдите, чтобы сохранить',
    save: 'Сохранить',
    saveAs: 'Сохранить как',
    export: 'Экспорт',
    loginToExport: 'Войдите, чтобы экспортировать',
    resetStyles: 'Сбросить стили',
    randomStylesPack: 'Применить случайную палитру',
    randomPaletteLoadFailed: 'Не удалось загрузить случайные палитры. Попробуйте снова.',
    defaultLegendTitle: 'ОТНОШЕНИЕ ИНТЕНСИВНОСТИ',
    done: 'Готово',
    newLegendRangeName: 'Новый диапазон',
    unsavedGuard: {
      title: 'Несохранённые изменения',
      body: 'Несохранённые изменения будут потеряны, если вы покинете эту страницу.',
      discard: 'Отменить и уйти',
      saveAndLeave: 'Сохранить и уйти',
    },
    region: {
      sectionTitle: 'Выбор страны',
      placeholder: 'Выберите страну...',
      ariaLabel: 'Выбор страны',
      changeConfirmTitle: 'Сменить страну?',
      changeConfirmBody:
        'Все несохранённые изменения будут потеряны, если вы смените страну и затем сохраните проект.',
      changeOk: 'Сменить страну',
    },
    importData: {
      issues: {
        title: 'Некоторые данные не удалось прочитать',
        body: 'Этот файл импортирован не полностью. Вы можете поручить разбор ИИ-агенту, импортировать то, что удалось прочитать, или отменить и исправить файл самостоятельно.',
        skippedRows:
          'Строк пропущено: {{count}} — в них не удалось определить регион или значение (импортировано: {{parsed}}).',
        unmatchedRegions: 'Названий регионов не совпадает с этой картой: {{count}} — {{regions}}',
        andMore: 'и ещё {{count}}.',
        useAiParser: 'Попробовать ИИ-агента',
        importAnyway: 'Всё равно импортировать',
        declineHint:
          'Отмена оставит текущие данные карты без изменений, чтобы вы могли исправить файл и импортировать его снова.',
      },
      duplicates: {
        title: 'Найдены повторяющиеся регионы',
        body: 'Комбинаций «регион/период», встречающихся более одного раза: {{groups}} ({{rows}} лишних строк). Выберите способ объединения.',
        examples: 'Например: {{regions}}',
        strategyFirst: 'Оставить первую строку',
        strategyLast: 'Оставить последнюю строку',
        strategySum: 'Сложить значения',
        strategyAverage: 'Использовать среднее значение',
        hint: 'В статистических выгрузках регион часто повторяется для каждой разбивки (возрастная группа, пол, единица измерения). Первая строка обычно содержит итоговые значения.',
        confirm: 'Импортировать',
      },
      sectionTitle: 'Импорт данных',
      downloadTooltipEmpty: 'Добавьте или импортируйте данные, чтобы скачать текущий набор',
      downloadTooltip: 'Скачать текущий (примерный) набор данных',
      downloadAria: 'Скачать текущий (примерный) набор данных',
      manualTooltip: 'Ввести данные вручную',
      manualTooltipNoCountry: 'Сначала выберите страну',
      manualAria: 'Ввести данные вручную',
      switchToStatic: 'Переключить на статические данные',
      switchToDynamic: 'Переключить на динамические данные',
      selectCountryFirst: 'Сначала выберите страну',
      switchAriaToStatic: 'Переключить на статические данные',
      switchAriaToDynamic: 'Переключить на динамические данные',
      segmentedAria: 'Формат импорта данных',
      regionIdsNote: 'ID регионов должны точно совпадать с ожидаемыми ID в системе.',
      downloadLink: 'Скачать',
      downloadTooltipNoCountry: 'Сначала выберите страну, чтобы скачать пример данных',
      sampleNoteSuffix: 'пример данных, чтобы получить правильные ID и сопоставить подписи.',
      expectedFormat: 'ОЖИДАЕМЫЙ ФОРМАТ:',
      sheetsSyncDescription:
        'Включена живая синхронизация с этой Google Таблицей. Вот ваша ссылка для доступа.',
      sheetsSyncLoading: 'Обновление данных из Google Таблицы…',
      sheetsCopyUrlTooltip: 'Копировать URL в буфер обмена',
      sheetsUrlCopied: 'URL скопирован',
      sheetsTabId: 'Вкладка таблицы (gid): {{gid}}',
      editManuallyInTable: 'Редактировать вручную в таблице',
      editManuallyInText:
        '\u0420\u0435\u0434\u0430\u043a\u0442\u0438\u0440\u043e\u0432\u0430\u0442\u044c \u0432\u0440\u0443\u0447\u043d\u0443\u044e \u0432 \u0442\u0435\u043a\u0441\u0442\u0435',
      aiParserChronographerTooltip:
        '\u0418\u0418-\u0430\u0433\u0435\u043d\u0442 \u0434\u043e\u0441\u0442\u0443\u043f\u0435\u043d \u0442\u043e\u043b\u044c\u043a\u043e \u043d\u0430 \u0442\u0430\u0440\u0438\u0444\u0435 {{badgeName}}.',
      connectSheets: 'Подключить Google Таблицы',
      changeSheetsSource: 'Сменить источник Google Таблиц',
      uploadCsv: 'Загрузить CSV-файл',
      uploadExcel: 'Загрузить Excel-файл',
      uploadJson: 'Загрузить JSON-файл',
      format: {
        csv: 'CSV',
        excel: 'Excel',
        json: 'JSON',
        sheets: 'Google Sheets',
        table: 'Таблица (вручную)',
        tabDelimited: 'Текст с разделителями (вручную)',
        aiParser: 'ИИ-агент',
      },
    },
    importFormatTooltip: {
      id: 'ID региона; должен точно совпадать с ожидаемыми ID (скачайте пример данных, чтобы их получить)',
      label: 'Подпись региона на карте',
      value: 'Числовое значение региона',
    },
    importFormatExamples: {
      excelColumnsStatic: 'Файл Excel со столбцами:',
      excelColumnsTime: 'Файл Excel со столбцами, включая столбец времени:',
      sheetsColumnsStatic: 'Google Таблица со столбцами:',
      sheetsColumnsTime: 'Google Таблица со столбцом времени:',
      manualStatic: 'Введите ID регионов, подписи и значения в форме.',
      manualHistorical: 'Введите ID регионов, подписи и значения с периодами времени в форме.',
      tabDelimitedStatic: 'Edit tab-separated rows. Existing data is pre-loaded for editing.',
      tabDelimitedHistorical:
        'Edit tab-separated rows with a time column. Existing data is pre-loaded for editing.',
      aiParserStatic: 'Describe your data or paste raw text and let AI structure it for you.',
    },
    switchMode: {
      body: 'Все изменения набора данных будут потеряны. Продолжить?',
      hint: 'Сначала сохраните текущий проект и при необходимости создайте новый, чтобы сохранить эти данные.',
    },
    googleSheets: {
      title: 'Импорт из Google Таблиц',
      intro: 'Вставьте URL публичной Google Таблицы с доступом «Все, у кого есть ссылка».',
      placeholder: 'https://docs.google.com/spreadsheets/d/...',
      howToShare: 'КАК ПОДЕЛИТЬСЯ:',
      stepOpen: 'Откройте таблицу',
      stepShare: 'Нажмите «Настройки доступа» → общий доступ',
      stepAnyone: 'Выберите «Все, у кого есть ссылка»',
      stepPaste: 'Скопируйте URL и вставьте выше',
      syncButton: 'Связать и синхронизировать',
      importOnceButton: 'Импорт один раз',
      fetching: 'Загрузка...',
      fetchFailed: 'Не удалось загрузить Google Таблицу',
    },
    embed: {
      title: 'Публичная вставка',
      modalTitle: 'Публичная вставка карты',
      openButton: 'Вставка',
      intro:
        'Поделитесь картой только для чтения на своём сайте через iframe или в виде отдельной размещённой страницы.',
      publicToggle: 'Включить публичную вставку',
      seoTitle: 'Заголовок страницы (SEO)',
      titleMax: 'Не более 200 символов',
      titleRequired: 'Заголовок страницы обязателен, если включена публичная вставка',
      seoDescription: 'Мета-описание (SEO)',
      descriptionMax: 'Не более 150 символов',
      descriptionRequired: 'Мета-описание обязательно, если включена публичная вставка',
      allowedOrigins: 'Доверенные origin',
      allowedOriginsHint:
        'Необязательно. Добавьте origin, которым разрешено встраивать карту (например: https://example.com). Разрешены только точные origin; wildcard (*) не поддерживается.',
      allowedOriginsPlaceholder: 'https://example.com',
      allowedOriginsInvalid:
        'Каждый origin должен быть валидным http(s)-origin (например: https://example.com) без пути, query, hash и *',
      allowedOriginsNoData: 'Нажмите Enter, чтобы добавить этот origin.',
      allowedOriginsAddCurrent: 'Добавить текущий origin',
      allowedOriginsAllowAll: 'Разрешить встраивание с любых origin',
      allowedOriginsAllowAllWarning:
        'Любой может встроить эту карту на любом сайте. Убедитесь, что содержимое подходит для публичного доступа, или ограничьте доступ, отключив этот переключатель и добавив конкретные origin в список доверенных.',
      allowedOriginsRequired:
        'Требуется хотя бы один разрешённый origin. Добавьте его или включите «Разрешить встраивание с любых origin».',
      seoKeywords: 'Meta-ключевые слова (необязательно)',
      keywordsHint: 'Необязательно. До 5 ключевых слов (ввод и Enter или запятая).',
      keywordPlaceholder: 'Введите ключевое слово и нажмите Enter.',
      titlePlaceholder: 'напр. {{example}}',
      descriptionPlaceholder:
        'напр. Интерактивная региональная карта «{{projectName}}» на Regionify.',
      defaultMetaDescription: 'Интерактивная региональная карта «{{projectName}}» на Regionify.',
      saveToGenerateLink:
        'Сохраните настройки, чтобы получить ссылку на публичную страницу и код вставки (iframe).',
      badgeRequired: 'Публичная вставка доступна на тарифе {{badgeName}}.',
      tooltipChronographerBody:
        'Публичная вставка карты (ссылка общего доступа и iframe) входит в тариф {{badgeName}}.',
      upgradeBadgesLink: 'Тарифы и апгрейд',
      tooltipNeedSavedProject: 'Сначала сохраните карту как проект, чтобы настроить вставку.',
      tooltipSelectCountry: 'Выберите страну перед использованием вставки.',
      tagSelectNoData: 'Нет совпадений. Введите ключевое слово и нажмите Enter.',
      saveSuccess: 'Настройки вставки сохранены',
      saveFailed: 'Не удалось сохранить настройки вставки',
      publicPage: 'Публичная страница',
      copyUrl: 'Ссылка',
      embedIframeCode: 'Код вставки (iframe)',
      copyEmbed: 'Код встраивания',
      copied: '{{label}} скопировано в буфер обмена',
      copyFailed: 'Не удалось скопировать',
      embedNotFoundTitle: 'Извините. Запрашиваемая страница недоступна.',
      embedNotFoundDescription:
        'Ссылка может быть отключена, просрочена или некорректна. Если проект существует и у вас есть к нему доступ, попробуйте открыть проект и снова включить публичное встраивание.',
      layout: 'Вёрстка',
      showHeader: 'Показывать заголовок на встроенной странице',
      showHeaderHint: 'Если включено, заголовок и описание страницы отображаются над картой.',
      embedErrorMessage: 'Что-то пошло не так, попробуйте позже или ',
      embedErrorContactUs: 'свяжитесь с нами',
    },
    exportModal: {
      title: 'Настроить экспорт',
      cropAndDownload: 'Обрезать и скачать',
      nextCropAndDownload: 'Далее: Обрезать и скачать',
      exportTypeLabel: 'Тип экспорта:',
      qualityLabel: 'Качество (%):',
      secondsPerPeriod: 'Секунд на период:',
      smoothTransitions: 'Плавные переходы:',
      animationDuration: '{{frames}} кадров • ~{{seconds}}с длительность',
      resolutionTierLabel: 'Разрешение',
      tierOriginal: 'Оригинал',
      tierHighResLocked: '2K and 4K exports require an Explorer or Chronographer badge.',
      aspectRatioLabel: 'Соотношение сторон:',
      aspectRatioPresetFree: 'Свободно',
      resolutionLabel: 'Разрешение (px):',
      width: 'Ширина',
      height: 'Высота',
      back: 'Назад',
      downloadFormat: 'Скачать {{format}}',
      upgradeToExplorer: 'Перейти на {{badgeName}}',
      singleFormatHintAfterLink: ' для PNG, SVG, полноценного экспорта статики без водяного знака.',
      qualityLimited: 'Качество ограничено {{max}}%.',
      qualityFullHint: ' для 100%.',
      exportFramesProcessed: 'Обработано кадров: {{processed}} из {{total}}',
      exportSvgBuilding: 'Создание SVG…',
      exportPdfBuilding: 'Создание PDF…',
      pdfPageFormatLabel: 'Формат страницы:',
      pdfFormatA4: 'A4',
      pdfFormatA3: 'A3',
      pdfFormatLetter: 'Letter',
      pdfFormatLegal: 'Legal',
      pdfOrientationLabel: 'Ориентация:',
      pdfOrientationPortrait: 'Книжная',
      pdfOrientationLandscape: 'Альбомная',
    },
    manualEntry: {
      title: 'Ручной ввод данных',
      clearAll: 'Очистить всё',
      addRow: 'Добавить строку',
      addMissingRow: 'Добавить недостающую строку',
      showOnChart: 'Показать на карте',
      hideFromChart: 'Скрыть с карты',
      duplicateRowsStatic:
        'Каждый ID региона должен быть уникальным. Удалите дубликаты строк перед сохранением.',
      duplicateRowsTimeline:
        'Каждый регион должен быть уникален в пределах периода. Удалите дубликаты строк перед сохранением.',
      placeholderRegionId: 'ID региона',
      placeholderLabel: 'Подпись',
      colIndex: '#',
      colId: 'ID',
      colLabel: 'Подпись',
      colValue: 'Значение',
      colTime: 'Время',
      colActions: 'Действия',
      deleteRow: 'Удалить строку',
      selectAll: 'Выбрать все строки',
      bulkActions: 'Действия с выбранным',
      bulkRemoveSelected: 'Удалить выбранные строки',
      bulkHideSelected: 'Скрыть выбранные на карте',
      bulkShowSelected: 'Показать выбранные на карте',
      filterClose: 'Готово',
      filterReset: 'Сброс',
      filterPlaceholder: {
        id: 'Поиск по ID',
        label: 'Поиск по подписи',
        value: 'Поиск по значению',
        time: 'Поиск по времени',
      },
      googleSheetsSyncReadOnlyNote:
        'Проект синхронизирует данные с привязанной Google-таблицей, поэтому здесь нельзя вручную редактировать значения. Изменяйте данные в таблице или выберите другой тип импорта, если нужно редактировать в приложении.',
    },
    tabDelimitedModal: {
      title: 'Текст с разделителями-табуляцией',
      placeholder: 'Редактируйте строки с табуляцией (id, label, value — по одной в строке)',
      pasteFormatTooltip:
        'Expected columns: id, label, value — or include a time column (year, time, period, …).\n\nExample:\nid\tlabel\tvalue\nUS-TX\tTexas\t42\nUS-CA\tCalifornia\t100\n\nWith time:\nid\tlabel\tvalue\tyear\nUS-TX\tTexas\t40\t2020\nUS-TX\tTexas\t42\t2021',
      pasteFormatAria: 'Ожидаемый формат вставки',
      pasteFormatNote:
        '\u041e\u0436\u0438\u0434\u0430\u0435\u043c\u044b\u0435 \u0441\u0442\u043e\u043b\u0431\u0446\u044b: id, label, value \u2014 \u0438\u043b\u0438 \u0434\u043e\u0431\u0430\u0432\u044c\u0442\u0435 \u0441\u0442\u043e\u043b\u0431\u0435\u0446 \u0432\u0440\u0435\u043c\u0435\u043d\u0438 (year, time, period, \u2026).',
      copyExampleTooltip:
        '\u041a\u043e\u043f\u0438\u0440\u043e\u0432\u0430\u0442\u044c \u043f\u0440\u0438\u043c\u0435\u0440',
      pasteFormatError: 'Текст не соответствует ожидаемому формату.',
      pasteMissingId: 'В первой строке нужен столбец id (см. пример).',
    },
    aiParserModal: {
      title: 'ИИ-агент',
      limitedRequestsNote: 'Не более {{max}} запросов к ИИ в сутки. Сегодня осталось: {{count}}.',
      aiAccuracyNote:
        'Результаты, сгенерированные ИИ, не всегда точны. Проверьте их перед сохранением.',
      placeholder:
        'Вставьте «грязные» данные — ИИ приведёт их к ожидаемому формату (id, label, value, опционально time)…',
      submit: 'Анализировать с ИИ',
      parsing: 'Обработка…',
      parseError: 'Ошибка ИИ-анализа. Пожалуйста, попробуйте снова.',
      parserEmptyResult: 'ИИ не вернул пригодных строк. Уточните ввод и попробуйте снова.',
      limitReached: 'Достигнут дневной лимит запросов к ИИ. Сбрасывается через 24 часа.',
      inputTooLong: 'Ввод превышает {{max}} символов.',
      switchToTable: 'Перейти к табличному виду',
      switchToAi: 'Перейти к ИИ-виду',
      tableEmpty: 'Пока нет строк. Добавьте строку вручную или запустите ИИ.',
      tableAddRow: 'Добавить строку',
      columnId: 'ID',
      columnLabel: 'Метка',
      columnValue: 'Значение',
      columnTime: 'Период',
      tabParser: 'Парсер',
      tabGenerator: 'Генератор',
      generatorPlaceholder:
        'Опишите набор данных для генерации (напр.: «Население стран ЕС в 2023 году»)…',
      generatorSubmit: 'Сгенерировать с ИИ',
      generatorParsing: 'Генерация…',
      generatorError: 'Ошибка генерации ИИ. Пожалуйста, попробуйте снова.',
      generatorEmptyResult: 'ИИ не вернул пригодных строк. Уточните запрос и попробуйте снова.',
      normalizeRangesPromptTitle: 'Нормализовать диапазоны легенды?',
      normalizeRangesPromptBody:
        'Импортированные данные могут иметь другой диапазон минимума/максимума, чем текущая легенда. Нормализовать диапазоны, чтобы они равномерно соответствовали новым данным?',
      normalizeRangesPromptHint:
        'Это равномерно перераспределит границы диапазонов между новым минимумом и максимумом. Названия и цвета диапазонов останутся прежними.',
    },
    legendConfig: {
      sectionTitle: 'Настройка легенды',
      sortAscending: 'Сортировка по возрастанию',
      sortDescending: 'Сортировка по убыванию',
      expandEdit: 'Развернуть для редактирования',
      expandEditAria: 'Развернуть для редактирования',
      normalizeRanges: 'Нормализовать диапазоны',
      normalizeRangesAria: 'Равномерно распределить диапазоны между текущими min и max',
      normalizeRangesNoData: 'Нет числовых данных для нормализации',
      addRange: 'Добавить диапазон',
      addRangeAria: 'Добавить диапазон легенды',
      collapseRanges: 'Диапазоны',
      collapseTheme: 'Тема',
      paletteSuggestions: 'Предложенные палитры',
      applyPalette: 'Применить палитру {{name}}',
      paletteGroups: {
        smooth: 'Плавные',
        creative: 'Креативные',
        highContrast: 'Высокий контраст',
      },
    },
    legendModal: {
      title: 'Управление диапазонами легенды',
      sortAscending: 'Сортировка по возрастанию',
      sortDescending: 'Сортировка по убыванию',
      addRange: 'Добавить диапазон',
    },
    legendColumns: {
      name: 'Имя',
      min: 'Мин',
      max: 'Макс',
      minLong: 'Мин. значение',
      maxLong: 'Макс. значение',
      color: 'Цвет',
    },
    mapStyles: {
      sectionTitle: 'Стили карты',
      collapseBackground: 'Фон',
      collapseBorder: 'Граница',
      collapseShadow: 'Тень',
      collapseControls: 'Элементы управления',
      collapseRegionLabels: 'Подписи регионов',
      freeBadgeNoteBeforeUpgrade:
        'Вы по-прежнему можете менять цвет фона, но прозрачный фон и снятие водяного знака требуют ',
      freeBadgeUpgradeLink: 'апгрейда значка',
      freeBadgeNoteAfterUpgrade: '.',
      showWatermark: 'Показывать водяной знак',
      transparent: 'Прозрачный',
      color: 'Цвет',
      showBorder: 'Показывать границу',
      width: 'Толщина',
      showShadow: 'Показывать тень',
      blur: 'Размытие',
      offsetX: 'Смещение X',
      offsetY: 'Смещение Y',
      showControls: 'Показывать элементы управления',
      tooltipZoomIn: 'Увеличить',
      tooltipZoomOut: 'Уменьшить',
      tooltipResetMapAndLabels: 'Сбросить карту и подписи',
      tooltipResetMap: 'Сбросить карту',
      tooltipEnableLabelDragging:
        'Включить перетаскивание подписей регионов (перетаскивание карты отключается)',
      tooltipDisableLabelDragging:
        'Отключить перетаскивание подписей регионов (перетаскивание карты снова включается)',
      tooltipPanUp: 'Переместить карту вверх',
      tooltipPanDown: 'Переместить карту вниз',
      tooltipPanLeft: 'Переместить карту влево',
      tooltipPanRight: 'Переместить карту вправо',
      showLabels: 'Показывать подписи',
      fontSize: 'Размер шрифта',
    },
    legendStyles: {
      sectionTitle: 'Стили легенды',
      collapseTitle: 'Заголовок',
      collapseLabels: 'Подписи',
      collapsePosition: 'Положение',
      showTitle: 'Показывать заголовок',
      titleField: 'Текст заголовка',
      titlePlaceholder: 'Введите заголовок',
      textColor: 'Цвет текста',
      fontSize: 'Размер шрифта',
      noDataColor: 'Цвет «нет данных»',
      positionFloating: 'Плавающая',
      positionBottom: 'Снизу',
      positionHidden: 'Скрыта',
      floatingHint: 'Перетащите и измените размер легенды на карте.',
    },
  },
  account: {
    title: 'Аккаунт',
    subtitle: 'Обновите своё имя и языковые настройки.',
    name: 'Имя',
    email: 'Эл. почта',
    emailNote: 'Электронную почту нельзя изменить.',
    language: 'Язык',
    saveProfile: 'Сохранить профиль',
    profileUpdated: 'Профиль успешно обновлён.',
    updateError: 'Не удалось обновить профиль',
    backToProjects: '← Назад к проектам',
  },
  contact: {
    title: 'Связаться с нами',
    subtitle: 'Остались вопросы? Мы здесь, чтобы помочь.',
    success: 'Сообщение отправлено! Мы скоро с вами свяжемся.',
    error: 'Не удалось отправить сообщение',
    firstName: 'Имя',
    lastName: 'Фамилия',
    email: 'Эл. почта',
    message: 'Сообщение',
    firstNameRequired: 'Введите имя',
    lastNameRequired: 'Введите фамилию',
    emailRequired: 'Введите адрес эл. почты',
    messageRequired: 'Введите сообщение',
    submit: 'Отправить',
  },
  projects: {
    title: 'Мои проекты',
    searchPlaceholder: 'Поиск проектов...',
    newProject: 'Новый проект',
    empty: 'Пока нет проектов. Создайте первую карту!',
    emptyFiltered: 'По вашему запросу ничего не найдено.',
    cardNoCountry: 'Без страны',
    cardCreated: 'Создано {{date}}',
    cardUpdated: 'Обновлено {{date}}',
    dataSource: 'Данные: {{type}}',
    dataSourceNone: 'Нет набора данных',
    cardRegionThumbnailAlt: 'Предпросмотр региональной карты',
    limitReachedTitle: 'Достигнут лимит проектов',
    limitReachedContent:
      'Вы достигли лимита бесплатного бейджа Observer в {{count}} проектов. Удалите хотя бы один проект, чтобы создать новый, или перейдите на более высокий бейдж.',
    selectAll: 'Выбрать все',
    sortAria: 'Сортировка проектов',
    sortUpdatedDesc: 'Обновлено (сначала новые)',
    sortUpdatedAsc: 'Обновлено (сначала старые)',
    sortCreatedDesc: 'Создано (сначала новые)',
    sortCreatedAsc: 'Создано (сначала старые)',
    sortNameAsc: 'Имя (А–Я)',
    sortNameDesc: 'Имя (Я–А)',
    deleteSelected: 'Удалить выбранные',
    deleteSelectedDisabledTooltip: 'Выберите один или несколько проектов для удаления',
    bulkSelectMaxReached: 'За раз можно выбрать не более {{max}} проектов.',
    bulkSelectTruncated: 'Выбраны только первые {{max}} проектов в этом списке (максимум за раз).',
    bulkSelectCapTooltip: 'Для массового удаления можно выбрать не более {{max}} проектов.',
    selectProjectAria: 'Выбрать проект {{name}}',
    lock: 'Заблокировать',
    unlock: 'Разблокировать',
    lockedTag: 'Заблокирован',
    lockedRenameTooltip: 'Проект заблокирован. Разблокируйте его, чтобы переименовать.',
    lockedDeleteTooltip: 'Проект заблокирован. Разблокируйте его, чтобы удалить.',
    bulkSelectLockedTooltip: 'Заблокированные проекты нельзя выбрать для удаления.',
  },
  common: {
    loading: 'Загрузка...',
    save: 'Сохранить',
    cancel: 'Отмена',
    delete: 'Удалить',
    rename: 'Переименовать',
    error: 'Что-то пошло не так',
    language: 'Язык',
    modalFullscreenEnter: 'Развернуть на весь экран',
    modalFullscreenExit: 'Выйти из полноэкранного режима',
  },
  messages: {
    projectSaved: 'Проект сохранён',
    projectSaveFailed: 'Не удалось сохранить проект',
    projectCreated: 'Проект создан',
    projectCreateFailed: 'Не удалось создать проект',
    projectDeleted: 'Проект удалён',
    projectDeleteFailed: 'Не удалось удалить проект',
    projectRenamed: 'Проект переименован',
    projectRenameFailed: 'Не удалось переименовать проект',
    sessionExpired: 'Сессия истекла. Пожалуйста, войдите снова.',
    projectsLoadFailed: 'Не удалось загрузить проекты',
    projectLoadFailed: 'Не удалось загрузить проект',
    mapExportedAs: 'Карта экспортирована в {{format}}',
    exportFailed: 'Ошибка экспорта. Попробуйте снова.',
    importHistoricalFirst:
      'Сначала импортируйте данные с временной колонкой для экспорта анимаций.',
    loggedInSuccess: 'Вход выполнен успешно!',
    enterEmail: 'Введите адрес электронной почты.',
    resendVerificationFailed: 'Не удалось отправить письмо с подтверждением повторно.',
    feedbackThankYou: 'Спасибо за отзыв!',
    noDataToDownload: 'Нет данных для загрузки. Добавьте или импортируйте данные.',
    downloadDataFailed: 'Не удалось загрузить данные. Попробуйте снова.',
    switchedToStatic: 'Переключено на статические данные. Экспорт в PNG, SVG или JPEG.',
    switchedToDynamic:
      'Переключено на динамические данные. Добавьте периоды вручную или импортируйте снова для экспорта GIF/Video.',
    switchToStaticConfirm: 'Переключить на статические данные?',
    switchToDynamicConfirm: 'Переключить на динамические данные?',
    switch: 'Переключить',
    importedRegions: 'Импортировано регионов: {{count}}',
    importedRowsPeriods: 'Импортировано {{count}} строк за {{periods}} периодов',
    timeSeriesDetected:
      'Обнаружены данные временных рядов. Перейдите на тариф {{badgeName}} для анимаций.',
    noTimeColumnDetected:
      'Временная колонка не обнаружена (year, time, period и т.д.). Данные импортированы как один период.',
    datasetMustIncludeId: 'В наборе данных должна быть колонка id.',
    dataFormatMismatch:
      'Данные не соответствуют ожидаемому формату. Скачайте образец для правильных ID.',
    failedParseExcel: 'Не удалось прочитать файл Excel. Скачайте образец для правильного формата.',
    failedParseFile: 'Не удалось прочитать файл. Скачайте образец для правильного формата.',
    noValidDataExcel: 'В файле Excel нет допустимых данных',
    noValidDataFile: 'В файле нет допустимых данных',
    missingColumns:
      'Некоторые столбцы не распознаны. Убедитесь, что первая строка набора данных содержит: id, label, value',
    downloadSampleFailed: 'Не удалось скачать образец. Попробуйте снова.',
    downloadSample: 'Скачать образец',
    close: 'Закрыть',
    deleteProjectTitle: 'Удалить проект',
    deleteProjectContent:
      'Вы уверены, что хотите удалить «{{name}}»? Это действие нельзя отменить.',
    deleteProjectEmbedWarningTitle: 'Публичная встройка включена',
    deleteProjectEmbedWarningContent:
      'Этот проект опубликован как публичная встройка. При удалении ссылка на встройку будет отключена навсегда — если карта встроена на каком-либо сайте, она перестанет там работать.',
    deleteProjectsBulkTitle: 'Удалить проекты',
    deleteProjectsBulkContent:
      'Удалить {{count}} выбранных проектов? Это действие нельзя отменить.',
    deleteProjectsBulkEmbedWarningTitle: 'Публичная встройка включена',
    deleteProjectsBulkEmbedWarningContent:
      'Проектов с публичной встройкой среди выбранных: {{count}}. При удалении эти ссылки на встройку будут отключены навсегда — если карты встроены на каких-либо сайтах, они перестанут там работать.',
    deleteProjectOk: 'Удалить',
    lockProjectTitle: 'Заблокировать проект',
    lockProjectContent:
      'Заблокировать «{{name}}»? Блокировка защищает проект, который вы считаете готовым.',
    lockProjectInfoTitle: 'Пока проект заблокирован, изменить его нельзя',
    lockProjectInfoContent:
      'Сохранение, переименование, изменение настроек вставки и удаление заблокированы. Вы по-прежнему можете открывать проект, настраивать его на экране и экспортировать изображения — но ничего не сохранится, пока вы не разблокируете его. Публичная вставка продолжит работать как обычно.',
    lockProjectOk: 'Заблокировать проект',
    unlockProjectTitle: 'Разблокировать проект',
    unlockProjectContent: 'Разблокировать «{{name}}»? Изменения и удаление снова станут возможны.',
    unlockProjectWarningTitle: 'Защита будет снята',
    unlockProjectWarningContent:
      'Проект больше не будет защищён от случайных изменений или удаления.',
    unlockProjectOk: 'Разблокировать проект',
    projectLocked: 'Проект заблокирован',
    projectUnlocked: 'Проект разблокирован',
    projectLockFailed: 'Не удалось изменить состояние блокировки. Попробуйте ещё раз.',
    projectLockedError: 'Проект заблокирован. Разблокируйте его, чтобы вносить изменения.',
    projectsBulkDeleted: 'Удалено проектов: {{count}}',
    projectsBulkDeletePartial: 'Удалено проектов: {{count}}. Заблокированные проекты пропущены.',
    projectsBulkDeleteFailed: 'Не удалось удалить выбранные проекты',
    loginFailed: 'Не удалось войти',
    signUpFailed: 'Не удалось создать аккаунт',
    sessionLimitReached:
      'Ваш аккаунт уже активен на максимальном количестве устройств ({{maxSessions}}). Сначала выйдите из системы на другом устройстве.',
    googleAuthFailed: 'Не удалось войти через Google. Пожалуйста, попробуйте ещё раз.',
  },
  footer: {
    terms: 'Условия использования',
    privacy: 'Политика конфиденциальности',
    refund: 'Политика возврата',
    contact: 'Контакт',
    faq: 'FAQ',
  },
  legal: {
    englishOnlyNote:
      'Эта страница доступна только на английском языке, чтобы сохранить единую юридически значимую версию.',
  },
  errors: {
    mapRenderFailed: 'Извините, не удалось отобразить карту.',
    panelFailed: 'Не удалось загрузить эту панель.',
    appCrashed: 'Что-то пошло не так.',
    reload: 'Перезагрузить',
    goHome: 'На главную',
    contactSupport: 'Связаться с поддержкой',
  },
};

export default ru;
